...
class MeshyRefineGenerator:
                    ...
                    for i in range(100):
                        ...
                        if status == "SUCCEEDED":
                            ...
                            if glb_url:
                                ...
                                # STRICT quality check - show results
                                if thumbnail_url and attempt < max_attempts - 1:
                                    quality_result = self._check_quality(thumbnail_url, user_request)
                                    
                                    # Show what I decided
                                    if not quality_result.get("approved", False):
                                        st.error(f"🚫 REJECTED: {quality_result.get('reason', 'Unknown issue')}")
                                        st.warning("🔄 Regenerating with different parameters...")
                                        break  # Try again
                                    else:
                                        st.success(f"✅ APPROVED: {quality_result.get('reason', 'Looks good')}")
    ...
    def _check_quality(self, thumbnail_url: str, original_prompt: str) -> Dict:
        """STRICT quality check - I actually look at it and decide"""
        try:
            ...
            response = self.anthropic_client.messages.create(
                ...
                messages=[{
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": f"""CRITICAL QUALITY CHECK for: "{original_prompt}"

Examine this 3D model carefully and honestly.

REJECT if ANY of these are true:
❌ Wrong object type (asked for A, got B)
❌ Missing critical parts (no legs, no handle, no mounting hole)
❌ Deformed/broken geometry
❌ Disconnected parts floating
❌ Unusable/non-functional
❌ Low quality/abstract/blob-like

APPROVE only if:
✅ Correct object type
✅ All parts present and connected
✅ Functional and usable
✅ Good quality geometry
✅ Would actually work for its purpose

Be HONEST and STRICT. The user is paying for quality.

Respond with JSON:
{{"approved": true/false, "reason": "specific explanation of what's wrong or right", "confidence": "high/medium/low"}}"""
                        }
                    ]
                }]
            )
            ...
            return {"approved": False, "reason": "Quality check failed to parse", "confidence": "low"}
            
        except Exception as e:
            # On error, reject to be safe
            return {"approved": False, "reason": f"Quality check error: {e}", "confidence": "low"}
